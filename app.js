...
function adjustSizes(total) {
    ...
    if (total <= 10) {
        minWidth = isPhone ? '90px'  : (isTablet ? '150px' : '140px');
        height   = isPhone ? '100px' : (isTablet ? '150px' : '140px');
        fontSize = isPhone ? '0.9rem'  : (isTablet ? '1.1rem'  : '1.1rem');
        chapterSize = isPhone ? '2.2rem' : (isTablet ? '2.8rem' : '2.5rem');
        padding  = isPhone ? '0.5rem'  : (isTablet ? '1rem'    : '1rem');
    } else if (total <= 20) {
        minWidth = isPhone ? '85px'  : (isTablet ? '130px' : '110px');
        height   = isPhone ? '90px'  : (isTablet ? '130px' : '110px');
        fontSize = isPhone ? '0.8rem'  : (isTablet ? '1rem'    : '1rem');
        chapterSize = isPhone ? '1.8rem' : (isTablet ? '2.4rem' : '2rem');
        padding  = isPhone ? '0.35rem' : (isTablet ? '0.7rem'  : '0.8rem');
    } else if (total <= 30) {
        minWidth = isPhone ? '78px'  : (isTablet ? '110px' : '95px');
        height   = isPhone ? '84px'  : (isTablet ? '110px' : '95px');
        fontSize = isPhone ? '0.74rem' : (isTablet ? '0.9rem'  : '0.9rem');
        chapterSize = isPhone ? '1.5rem' : (isTablet ? '1.9rem' : '1.6rem');
        padding  = isPhone ? '0.25rem' : (isTablet ? '0.5rem'  : '0.6rem');
    } else {
        minWidth = isPhone ? '70px'  : (isTablet ? '95px'  : '80px');
        height   = isPhone ? '76px'  : (isTablet ? '95px'  : '80px');
        fontSize = isPhone ? '0.65rem' : (isTablet ? '0.78rem' : '0.75rem');
        chapterSize = isPhone ? '1.2rem' : (isTablet ? '1.6rem' : '1.3rem');
        padding  = isPhone ? '0.18rem' : (isTablet ? '0.35rem' : '0.4rem');
    }
